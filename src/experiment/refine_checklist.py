...
def load_args():
    ...
    parser.add_argument(
        ...
        default="data/prompt/generate_checklist/refine_baseline.txt",
    )
    parser.add_argument(
        "--system-prompt",
        type=str,
...
def main():
    ...
    for question in tqdm(questions, desc="Generating checklist"):
        ...
        if not base_checklist:
            results.append({"question": question, "response": None, "checklist": None})
            continue
        text_checklist_items = "\n".join(
            [f"  {i+1}. {c}" for i, c in enumerate(base_checklist)]
        )
        prompt = base_prompt.format(question=question, checklist=text_checklist_items)
